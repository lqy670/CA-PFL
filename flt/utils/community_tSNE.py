...
if __name__ == '__main__':
    ...
    #     batch_size=4
    # )
    dataloader = DataLoader(dataset=test_dataset[1], batch_size=16, shuffle=True)
    ...
    pthfile2 = '../../exps/2023-06-04-1627-34/models/client7_round_50.pth'
    model2.load_state_dict(torch.load(pthfile2))
    model2.cuda()
    model2.eval()

    model3 = network.SimpleCNN(num_classes=10)
    pthfile3 = '../../exps/2023-06-04-1627-34/models/community1_round_50.pth'
    ...
    with torch.no_grad():
        ...
        for i, (x, y) in enumerate(dataloader):
            x, y = x.cuda(), y.cuda()
            output = forward_features(model,x)
    ...
    feature_bank2 = output_bank2.cpu().numpy()
    feature_bank3 = output_bank3.cpu().numpy()
    feature_bank4 = output_bank4.cpu().numpy()
    ...
    plt.subplots_adjust(left=0, bottom=0.05, right=1.0, top=1.0, wspace=0.03, hspace=0.12)
    ax1 = axs[0, 0]
    ax2 = axs[0, 1]
    ax3 = axs[1, 0]
    ax4 = axs[1, 1]
    ...
    ax1.set_title('(a) Client-a Model',y =-0.1)
    ax2.set_title('(b) Client-b Model',y =-0.1)
    ax3.set_title('(c) Community-A model',y =-0.1)
    ax4.set_title('(d) Global Model',y =-0.1)
