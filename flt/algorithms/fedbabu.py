...
class FedBabu(FedBase):
    ...
    def _train2(self, net, dataset, test_dataset, optimizer, bs: int, E: int, device: torch.device):
        train_dataloader = data.DataLoader(dataset=dataset, batch_size=bs, shuffle=True, drop_last=True)
        test_dataloader = data.DataLoader(dataset=test_dataset, batch_size=bs, shuffle=False, drop_last=True)
        ...
        for epoch in range(E):
            ...
            with torch.no_grad():
                ...
                for _, (x, y) in enumerate(test_dataloader):
                    ...
                    correct += (pred_label == y.data).sum().item()
            logging.info(
                f"    >>> [Local Fine tuning] Epoch: {epoch + 1}, Loss: {sum(epoch_loss_lst) / len(epoch_loss_lst)}, Acc: {correct / total}")
            last_acc = correct / total
    ...
    def start(self):
        # 全局模型下发
        for round in range(self._comm_round-1):
            ...
            logging.info(f"[Gloabl] Round: {round + 1}, Client model before training - Clients Average Acc: {step1_avg_acc / self._nk_parties}")
            # TODO step2.融合全局模型前所有本地模型在各个测试集上的的平均准确率
            logging.info(f"[Gloabl] Round: {round + 1}, Client model after training - Clients Average Acc: {step2_avg_acc / self._nk_parties}")
            # 模型聚合
        ...
        for idx, (key, net) in enumerate(samples.items()):
            ...
            logging.info(f"  >>> [Local fine-tuning] client: {key} / [{idx + 1}/{len(samples)}]")
            optimizer = self._optimizer(self._optim_name, net, lr=self._lr, weight_decay=self._weight_decay)
            net, last_acc = self._train2(
                net, dataset=self._datasets[key], test_dataset=self._test_dataset[key],
                optimizer=optimizer, bs=self._bs, E=3, device=self._device
            )
            step2_avg_acc += last_acc
        # TODO step1.每轮联邦学习开始时 使用全局模型更新本地模型后 本地模型在各个客户端的测试集上的准确率
        logging.info(
            f"[Gloabl] Client model before fine tuning - Clients Average Acc: {step1_avg_acc / self._nk_parties}")
        # TODO step2.融合全局模型前所有本地模型在各个测试集上的的平均准确率
        logging.info(
            f"[Gloabl] Client model after fine tuning - Clients Average Acc: {step2_avg_acc / self._nk_parties}")
