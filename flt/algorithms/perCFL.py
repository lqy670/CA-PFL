...
class PerCFL(FedBase):
    def __init__(
        ...
        self._clients_W_old = {}  # 缓存客户端模型训练前初始参数
    ...
    def start(self):
        ...
        for round in range(self._comm_round):
            ...
            logging.info(f">>> [Round] {round + 1} -- [CFL] {cluster_indices}")
            ...
            for idc in cluster_indices:
                ...
                logging.info(f"the max_norm is {max_norm}, the mean_norm is {mean_norm}")
            ...
            if len(cluster_indices_new) > len(cluster_indices):
                logging.info(f"    >>> [Round] {round + 1} -- [cluster Change] {cluster_indices_new}")
