...
def partition_at_level(dendrogram, level):
    ...
    generate_dendrogram : to obtain the partition of highest __modularity
...
def modularity(partition, graph, weight='weight'):
    ...
    for node in graph:
        com = partition[node]
        # deg[com]为对com中所有节点的邻边(包括跨边)边权进行求和
...
def best_partition(graph,
                   ...
                   randomize=None,
                   random_state=None):
    """Compute the partition of the graph nodes which maximises the __modularity
    (or try..) using the Louvain heuristices

    This is the partition of highest __modularity, i.e. the highest partition
